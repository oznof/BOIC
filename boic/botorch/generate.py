...
# modified gen_candidates_scipy: limit precision of value and grad (for reproducibility on different machines)
...
def gen_candidates_scipy(
    ...
    precision: Optional[Union[int, List[int]]] = None,
) -> Tuple[Tensor, Tensor]:
    ...
    def f(x, precision=precision):
        ...
        losses = -acquisition_function(X_fix)
        if precision is not None:
            is_int_precision = isinstance(precision, int)
            if torch.all(losses <= 0):
                eps = 10 ** -(precision if is_int_precision else precision[0])
                losses = torch.clamp_max(losses + eps, 0)
        loss = losses.sum()
        ...
        #print(fval)
        if precision is not None:
            fval = np.round(fval, precision if is_int_precision else precision[0])
            gradf = np.round(gradf, precision if is_int_precision else precision[1])
        return fval, gradf
