import numpy as np
from .rover_utils import RoverDomain, PointBSpline, ConstObstacleCost, NegGeom
from .rover_utils import AABoxes, UnionGeom, AdditiveCosts, ConstCost
...
def create_cost_large(change_c=None, cost_c=20.):
    ...
    costs = [ConstObstacleCost(obstacles, cost=float(cost_c)), ConstCost(0.05)]
    cost_fn = AdditiveCosts(costs)
    return cost_fn, start, goal
...
def create_large_domain(force_start=False,
                        ...
                        goal_miss_cost=None,
                        change_c=None,
                        new_start=None, new_goal=None,
                        n_points=30, noise=1e-4,
                        n_samples=1000, cost_c=20., clamp_traj=False,
                        bounds=np.array([[-0.1, -0.1], [1.1, 1.1]]),
                        deterministic_perturb=False):
    cost_fn, start, goal = create_cost_large(change_c=change_c, cost_c=cost_c)
    if new_start is not None:
        start = new_start
    if new_goal is not None:
        goal = new_goal
    traj = PointBSpline(dim=2, num_points=n_points, clamp=clamp_traj, bounds=bounds)
    n_params = traj.param_size
    domain = RoverDomain(cost_fn,
                         start=start,
                         goal=goal,
                         ...
                         s_range=bounds,
                         noise=noise, n_samples=n_samples, cost_c=cost_c, clamp_traj=clamp_traj,
                         deterministic_perturb=deterministic_perturb)
    return domain


def rover(n_points=30, force_start=False, force_goal=False):
    def l1cost(x, point):
        return 10 * np.linalg.norm(x - point, 1)

    domain = create_large_domain(force_start=force_start,
                                 force_goal=force_goal,
...
def boundary_penalty(c):
    lins = np.linspace(0, 1, 100)
    ## remove 1 box to promote trajectories that go through center
    ##c = np.vstack((c[:39], c[40:]))
    for i in [0, 0.05]:
        south = np.zeros((len(lins), 2)) + i
        south[:, 0] = lins
        north = np.ones((len(lins), 2)) - i
        north[:, 0] = lins
        west = np.zeros((len(lins), 2)) + i
        west[:, 1] = lins
        east = np.ones((len(lins), 2)) - i
        east[:, 1] = lins
        c = np.vstack((c, south, north, west, east))
    return c

def srover(n_points=30):
    def l1cost(x, point):
        return 10 * np.linalg.norm(x - point, 1)
    domain = create_large_domain(force_start=False,
                                 ...
                                 start_miss_cost=l1cost,
                                 goal_miss_cost=l1cost,
                                 change_c=boundary_penalty,
                                 new_start=np.array([0.2, 0.3]), new_goal=np.array([0.8, 0.7]),
                                 n_points=n_points)
    n_points = domain.traj.npoints
    raw_x_range = np.repeat(domain.s_range, n_points, axis=1)
    f_max = 5.0
...
def mrover(n_points=30, xycost=50, noise=1e-12, deterministic_pertub=True, s=0,
           n_samples=10000, cost_c=20, clamp_traj=True, new_start=None, new_goal=None):
    #            new_start=np.array([0.2, 0.3]), new_goal=np.array([0.8, 0.7]),
    def l1cost(x, point):
        return xycost * np.linalg.norm(x - point, 1)
    domain = create_large_domain(force_start=False,
                                 force_goal=False,
                                 start_miss_cost=l1cost,
                                 goal_miss_cost=l1cost,
                                 ...
                                 n_points=n_points,
                                 noise=noise, n_samples=n_samples, cost_c=cost_c, clamp_traj=clamp_traj,
                                 deterministic_perturb=deterministic_pertub)
    domain.traj._s = s
    n_points = domain.traj.npoints
    raw_x_range = np.repeat(domain.s_range, n_points, axis=1)
    f_max = 5.0
