# from https://github.com/zi-w/Ensemble-Bayesian-Optimization/blob/master/test_functions/rover_utils.py
import numpy as np
...
class PointBSpline(Trajectory):
    ...
    def __init__(self, dim, num_points, clamp=False, bounds=None):
        self.tck = None
        self.d = dim
    ...
    def set_params(self, params, start=None, goal=None):
        ...
        self.tck, u = si.splprep(points, k=3, s=self._s)
    ...
    def get_points(self, t):
        ...
        traj = np.vstack(si.splev(t, self.tck)).T
        if self._clamp:
            assert self._bounds is not None
            traj = np.clip(traj, a_min=self._bounds[0], a_max=self._bounds[1])
        return traj
...
class RoverDomain:
    ...
    def __call__(self, params):
        ...
        return -self.estimate_cost(n_samples=self.n_samples)

    def set_params(self, params):
        if self.noise is not None:
            rng =  np.random.RandomState(117) if self.deterministic_perturb else self.rnd_stream
            perturb = rng.normal(0, self.noise, params.shape)
            params = params + perturb
        self.traj.set_params(params,
